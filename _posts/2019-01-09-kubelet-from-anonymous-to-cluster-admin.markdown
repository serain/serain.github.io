...
Note that because of differences in headers that the `kube-apiserver` expects to see, it may be necessary to download a version of `kubectl` that matches the target Kubernetes cluster. The cluster version can be obtained anonymously by hitting the `/version` endpoint. The corresponding binary can then be fetched by modifying the version in the request below:
