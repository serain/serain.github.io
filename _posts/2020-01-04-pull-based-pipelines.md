...
And just like that, the state of your cluster is version controlled, auditable and protected from drift. You can go ahead and revoke everyone's direct access to the cluster; they don't need `kubectl` anymore.
...
IMAGE (engineer)->(github)->(circleCI)->(registry)
...
Traditional CI/CD tools are a security hazard. It's true that some offer better security controls than others, but either way, there are tangible security benefits with pull-based pipelines.

We need a pull-based tool for building image CD. The same should also be considered for other activities, like updating a common library on Artifactory.
